...
def spiral_waypoints(center_x, center_y, z, initial_radius, n_turns, n_points_per_turn, roll=0, pitch=0, yaw=0):
    waypoints = []
    qx, qy, qz, qw = quat_from_rpy(roll, pitch, yaw)
    total_points = n_turns * n_points_per_turn
    max_radius = initial_radius * n_turns
    for i, theta in enumerate(np.linspace(0, 2*math.pi*n_turns, total_points, endpoint=True)):
        radius = initial_radius + (max_radius - initial_radius) * (i / total_points)
        pose = Pose()
...
class SpiralTrajectoryClient(Node):
    ...
    def plan_and_execute_spiral(self):
        waypoints = spiral_waypoints(
            ...
            initial_radius=0.05, n_turns=3, n_points_per_turn=50,
        ...
        self.get_logger().info("Calcul de la trajectoire spirale via /compute_cartesian_path...")
        ...
        if exec_result.error_code.val == 1:
            self.get_logger().info("Trajectoire spirale exécutée ✅")
        else:
